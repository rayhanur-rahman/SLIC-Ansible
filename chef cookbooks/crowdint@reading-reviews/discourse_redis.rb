...
class DiscourseRedis
  ...
  def self.url(config=nil)
    config ||= self.config
    "redis://#{(':' + config['password'] + '@') if config['password']}#{config['host']}:#{config['port']}"
  end
  ...
  def self.new_redis_store
    ...
    redis_store = ActiveSupport::Cache::RedisStore.new "redis://#{(':' + redis_config['password'] + '@') if redis_config['password']}#{redis_config['host']}:#{redis_config['port']}"
