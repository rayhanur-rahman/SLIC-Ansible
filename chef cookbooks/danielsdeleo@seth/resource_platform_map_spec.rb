...
describe Seth::Resource::PlatformMap do

  before(:each) do
    @platform_map = Seth::Resource::PlatformMap.new({
      :windows => {
        "6.1" => {
          :file => "softiefile",
          :else => "thing"
      ...
      :default => {
        :soundwave => "lazerbeak",
        :directory => Seth::Resource::Directory,
      }
    })
  end
  ...
  describe 'finding a resource' do
    ...
    it "returns a default resource if platform and version don't exist" do
      @platform_map.get(:remote_file).should eql(Seth::Resource::RemoteFile)
    end
    ...
    it "returns a resource with a Seth::Resource object" do
  ...
  describe 'building the map' do
    it "allows passing of a resource map at creation time" do
      @new_map = Seth::Resource::PlatformMap.new({:the_dude => {:default => 'abides'}})
      @new_map.map[:the_dude][:default].should eql("abides")
    end

    it "defaults to a resource map with :default key" do
      @new_map = Seth::Resource::PlatformMap.new
