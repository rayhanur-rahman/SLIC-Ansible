...
describe Seth::sethFS::FilePattern do
  def p(str)
    Seth::sethFS::FilePattern.new(str)
  end

  # Different kinds of patterns
  context 'with empty pattern ""' do
    let(:pattern) { Seth::sethFS::FilePattern.new('') }
    it 'match?' do
  ...
  context 'with simple pattern "abc"' do
    let(:pattern) { Seth::sethFS::FilePattern.new('abc') }
    it 'match?' do
  ...
  context 'with simple pattern "abc/def/ghi"' do
    let(:pattern) { Seth::sethFS::FilePattern.new('abc/def/ghi') }
    it 'match?' do
  ...
  context 'with simple pattern "a\*\b"', :pending => (Seth::Platform.windows?) do
    let(:pattern) { Seth::sethFS::FilePattern.new('a\*\b') }
    it 'match?' do
  ...
  context 'with star pattern "/abc/*/ghi"' do
    let(:pattern) { Seth::sethFS::FilePattern.new('/abc/*/ghi') }
    it 'match?' do
      pattern.match?('/abc/def/ghi').should be_true
  ...
  context 'with star pattern "/abc/d*f/ghi"' do
    let(:pattern) { Seth::sethFS::FilePattern.new('/abc/d*f/ghi') }
    it 'match?' do
      pattern.match?('/abc/def/ghi').should be_true
  ...
  context 'with star pattern "/abc/d[a-z][0-9]f/ghi"', :pending => (Seth::Platform.windows?) do
    let(:pattern) { Seth::sethFS::FilePattern.new('/abc/d[a-z][0-9]f/ghi') }
    it 'match?' do
  ...
  context 'with star pattern "a**b**c"' do
    let(:pattern) { Seth::sethFS::FilePattern.new('a**b**c') }
    it 'match?' do
  ...
  context 'normalization tests' do
    ...
    it 'raises error on dotdot with overlapping double star' do
      lambda { Seth::sethFS::FilePattern.new('abc/**/../def').exact_path }.should raise_error(ArgumentError)
      lambda { Seth::sethFS::FilePattern.new('abc/**/abc/../../def').exact_path }.should raise_error(ArgumentError)
    end
