  ...
  class Win32
    class Registry
      ...
      def set_value(key_path, value)
        Seth::Log.debug("Updating value #{value[:name]} in registry key #{key_path} with type #{value[:type]} and data #{value[:data]}")
        key_exists!(key_path)
        hive, key = get_hive_and_key(key_path)
        if value_exists?(key_path, value)
          if data_exists?(key_path, value)
            Seth::Log.debug("Value #{value[:name]} in registry key #{key_path} already had those values, not updated")
            return false
          else
            ...
            Seth::Log.debug("Value #{value[:name]} in registry key #{key_path} updated")
          end
        else
          hive.open(key, ::Win32::Registry::KEY_SET_VALUE | ::Win32::Registry::KEY_QUERY_VALUE | registry_system_architecture) do |reg|
            reg.write(value[:name], get_type_from_name(value[:type]), value[:data])
          end
          Seth::Log.debug("Value #{value[:name]} in registry key #{key_path} created")
        end
        true
      end

      def delete_value(key_path, value)
        Seth::Log.debug("Deleting value #{value[:name]} from registry key #{key_path}")
        if value_exists?(key_path, value)
          begin
            hive, key = get_hive_and_key(key_path)
          rescue Seth::Exceptions::Win32RegKeyMissing
            return true
          end
          hive.open(key, ::Win32::Registry::KEY_SET_VALUE | registry_system_architecture) do |reg|
            reg.delete_value(value[:name])
            Seth::Log.debug("Deleted value #{value[:name]} from registry key #{key_path}")
          end
        else
          Seth::Log.debug("Value #{value[:name]} in registry key #{key_path} does not exist, not updated")
        end
        true
      end

      def create_key(key_path, recursive)
        Seth::Log.debug("Creating registry key #{key_path}")
        if keys_missing?(key_path)
          if recursive == true
            Seth::Log.debug("Registry key #{key_path} has missing subkeys, and recursive specified, creating them....")
            create_missing(key_path)
          else
            raise Seth::Exceptions::Win32RegNoRecursive, "Registry key #{key_path} has missing subkeys, and recursive not specified"
          end
        end
        if key_exists?(key_path)
          Seth::Log.debug("Registry key #{key_path} already exists, doing nothing")
        else
          hive, key = get_hive_and_key(key_path)
          hive.create(key, ::Win32::Registry::KEY_WRITE | registry_system_architecture)
          Seth::Log.debug("Registry key #{key_path} created")
        end
        true
      end

      def delete_key(key_path, recursive)
        Seth::Log.debug("Deleting registry key #{key_path}")
        unless key_exists?(key_path)
          Seth::Log.debug("Registry key #{key_path}, does not exist, not deleting")
          return true
        end
        ...
        if has_subkeys?(key_path)
          if recursive == true
            ...
            subkeys.each do |key|
              ...
              Seth::Log.debug("Deleting registry key #{key_path} recursively")
              delete_key(keypath_to_check, true)
            end
            delete_key_ex(hive_namespace, key_including_parent)
          else
            raise Seth::Exceptions::Win32RegNoRecursive, "Registry key #{key_path} has subkeys, and recursive not specified"
          end
        else
          delete_key_ex(hive_namespace, key_including_parent)
          return true
      ...
      def key_exists!(key_path)
        unless key_exists?(key_path)
          raise Seth::Exceptions::Win32RegKeyMissing, "Registry key #{key_path} does not exist"
        end
        true
      end
      ...
      def hive_exists?(key_path)
        begin
          hive, key = get_hive_and_key(key_path)
        rescue Seth::Exceptions::Win32RegHiveMissing => e
          return false
        end
        return true
      end
      ...
      # 32-bit seth clients running on 64-bit machines will default to reading the 64-bit registry
      ...
      def value_exists!(key_path, value)
        unless value_exists?(key_path, value)
          raise Seth::Exceptions::Win32RegValueMissing, "Registry key #{key_path} has no value named #{value[:name]}"
        end
        true
      end
      ...
      def data_exists!(key_path, value)
        unless data_exists?(key_path, value)
          raise Seth::Exceptions::Win32RegDataMissing, "Registry key #{key_path} has no value named #{value[:name]}, containing type #{value[:type]} and data #{value[:data]}"
        end
        true
      end
      ...
      def type_matches!(key_path, value)
        unless type_matches?(key_path, value)
          raise Seth::Exceptions::Win32RegTypesMismatch, "Registry key #{key_path} has a value #{value[:name]} with a type that is not #{value[:type]}"
        end
      end
      ...
      def assert_architecture!
        if machine_architecture == :i386 && architecture == :x86_64
          raise Seth::Exceptions::Win32RegArchitectureIncorrect, "cannot access 64-bit registry on a 32-bit windows instance"
        end
      end
      ...
      def get_hive_and_key(path)
        ...
        raise Seth::Exceptions::Win32RegHiveMissing, "Registry Hive #{hive_name} does not exist" unless hive
      ...
      def create_missing(key_path)
        ...
        missing_key_arr.each do |intermediate_key|
          ...
          if !key_exists?(existing_key_path)
            Seth::Log.debug("Recursively creating registry key #{existing_key_path}")
