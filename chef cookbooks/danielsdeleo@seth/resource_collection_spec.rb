...
describe Seth::ResourceCollection do

  before(:each) do
    @rc = Seth::ResourceCollection.new()
    @resource = Seth::Resource::ZenMaster.new("makoto")
  end

  describe "initialize" do
    it "should return a Seth::ResourceCollection" do
      @rc.should be_kind_of(Seth::ResourceCollection)
    end
  end

  describe "[]" do
    it "should accept Seth::Resources through [index]" do
      lambda { @rc[0] = @resource }.should_not raise_error
      lambda { @rc[0] = "string" }.should raise_error
    end

    it "should allow you to fetch Seth::Resources by position" do
      @rc[0] = @resource
  ...
  describe "push" do
    it "should accept Seth::Resources through pushing" do
  ...
  describe "insert" do
    it "should accept only Seth::Resources" do
    ...
    it "should append resources to the end of the collection when not executing a run" do
      zmr = Seth::Resource::ZenMaster.new("there is no spoon")
      @rc.insert(@resource)
      @rc.insert(zmr)
      @rc[0].should eql(@resource)
      @rc[1].should eql(zmr)
    ...
    it "should insert resources to the middle of the collection if called while executing a run" do
      resource_to_inject = Seth::Resource::ZenMaster.new("there is no spoon")
      zmr = Seth::Resource::ZenMaster.new("morpheus")
      dummy = Seth::Resource::ZenMaster.new("keanu reeves")
      @rc.insert(zmr)
      @rc.insert(dummy)
  ...
  describe "lookup" do
    it "should allow you to find resources by name via lookup" do
      zmr = Seth::Resource::ZenMaster.new("dog")
      @rc << zmr
      @rc.lookup(zmr.to_s).should eql(zmr)

      zmr = Seth::Resource::ZenMaster.new("cat")
      @rc[0] = zmr
      @rc.lookup(zmr).should eql(zmr)

      zmr = Seth::Resource::ZenMaster.new("monkey")
      @rc.push(zmr)
      @rc.lookup(zmr).should eql(zmr)
    end
    ...
    it "should raise an exception if it cannot find a resource with lookup" do
      lambda { @rc.lookup("zen_master[dog]") }.should raise_error(Seth::Exceptions::ResourceNotFound)
    end
  end
  ...
  describe "resources" do
    ...
    it "raises an error when attempting to find a resource that does not exist" do
      lambda {@rc.find("script[nonesuch]")}.should raise_error(Seth::Exceptions::ResourceNotFound)
    end
  ...
  describe "when validating a resource query object" do
    ...
    it "accepts a seth resource object" do
      res = Seth::Resource.new("foo", nil)
    ...
    it "rejects an argument that is not a String, Hash, or Seth::Resource" do
      lambda do
  ...
  describe "self.from_json" do
    it "should deserialize itself from json" do
      @rc << @resource
      json = @rc.to_json
      s_rc = Seth::JSONCompat.from_json(json)
      s_rc.should be_a_kind_of(Seth::ResourceCollection)
  ...
  def load_up_resources
    %w{dog cat monkey}.each do |n|
       @rc << Seth::Resource::ZenMaster.new(n)
    end
    @rc << Seth::Resource::File.new("something")
  end
