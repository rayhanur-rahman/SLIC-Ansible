...
describe Seth::ConfigFetcher do
  ...
  let(:fetcher) { Seth::ConfigFetcher.new(config_location, config_jail_path) }

  context "when loading a local file" do
    let(:config_location) { "/etc/seth/client.rb" }
    ...
    context "and consuming JSON" do
      ...
      let(:config_location) { "/etc/seth/first-boot.json" }
  ...
  context "when loading a file over HTTP" do
    ...
    before do
      Seth::HTTP::Simple.should_receive(:new).
        with(config_location).
        and_return(http)
    end
