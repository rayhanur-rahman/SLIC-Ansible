...
require 'seth/ceth/download'
require 'seth/ceth/diff'

describe 'ceth download' do
  ...
  context 'without versioned cookbooks' do
    when_the_seth_server "has one of each thing" do
      ...
      when_the_repository 'has only top-level directories' do
        ...
        it 'ceth download downloads everything' do
          ceth('download /').should_succeed <<EOM
...
EOM
      ...
      when_the_repository 'has an identical copy of each thing' do
        ...
        it 'ceth download makes no changes' do
          ceth('download /').should_succeed ''
          ceth('diff --name-status /').should_succeed ''
        end

        it 'ceth download --purge makes no changes' do
          ceth('download --purge /').should_succeed ''
          ceth('diff --name-status /').should_succeed ''
        end
...
EOM
          it 'ceth download changes the role' do
            ceth('download /').should_succeed "Updated /roles/x.json\n"
            ceth('diff --name-status /').should_succeed ''
          end

          it 'ceth download --no-diff does not change the role' do
            ceth('download --no-diff /').should_succeed ''
...
EOM
          it 'ceth download / does not change anything' do
            ceth('download /').should_succeed ''
            ceth('diff --name-status /').should_succeed ''
          end
        end
        ...
        context 'as well as one extra copy of each thing' do
          ...
          it 'ceth download does nothing' do
            ceth('download /').should_succeed ''
...
EOM
          ...
          it 'ceth download --purge deletes the extra files' do
            ceth('download --purge /').should_succeed <<EOM
...
EOM
      ...
      when_the_repository 'is empty' do
        it 'ceth download creates the extra files' do
          ceth('download /').should_succeed <<EOM
Created /clients
Created /clients/seth-validator.json
Created /clients/seth-webui.json
...
EOM
        ...
        it 'ceth download --no-diff creates the extra files' do
          ceth('download --no-diff /').should_succeed <<EOM
Created /clients
Created /clients/seth-validator.json
Created /clients/seth-webui.json
Created /clients/x.json
...
EOM
        ...
        context 'when current directory is top level' do
          cwd '.'
          it 'ceth download with no parameters reports an error' do
            ceth('download').should_fail "FATAL: Must specify at least one argument.  If you want to download everything in this directory, type \"ceth download .\"\n", :stdout => /USAGE/
          end
        end
      end
    end
    ...
    when_the_repository 'is empty' do
      when_the_seth_server 'has two data bag items' do
        ...
        it 'ceth download of one data bag item itself succeeds' do
          ceth('download /data_bags/x/y.json').should_succeed <<EOM
Created /data_bags
Created /data_bags/x
Created /data_bags/x/y.json
EOM
...
EOM
        ...
        it 'ceth download /data_bags/x /data_bags/x/y.json downloads x once' do
          ceth('download /data_bags/x /data_bags/x/y.json').should_succeed <<EOM
Created /data_bags
Created /data_bags/x
Created /data_bags/x/y.json
...
EOM
      ...
      when_the_seth_server 'has a modified, unmodified, added and deleted data bag item' do
        ...
        it 'ceth download of the modified file succeeds' do
          ceth('download /data_bags/x/modified.json').should_succeed <<EOM
...
EOM
        ...
        it 'ceth download of the unmodified file does nothing' do
          ceth('download /data_bags/x/unmodified.json').should_succeed ''
          ceth('diff --name-status /data_bags').should_succeed <<EOM
D\t/data_bags/x/added.json
...
EOM
        ...
        it 'ceth download of the added file succeeds' do
          ceth('download /data_bags/x/added.json').should_succeed <<EOM
...
EOM
        ...
        it 'ceth download of the deleted file does nothing' do
          ceth('download /data_bags/x/deleted.json').should_succeed ''
          ceth('diff --name-status /data_bags').should_succeed <<EOM
D\t/data_bags/x/added.json
M\t/data_bags/x/modified.json
A\t/data_bags/x/deleted.json
EOM
        ...
        it 'ceth download --purge of the deleted file deletes it' do
          ceth('download --purge /data_bags/x/deleted.json').should_succeed <<EOM
...
EOM
        ...
        it 'ceth download of the entire data bag downloads everything' do
          ceth('download /data_bags/x').should_succeed <<EOM
Created /data_bags/x/added.json
Updated /data_bags/x/modified.json
EOM
          ceth('diff --name-status /data_bags').should_succeed <<EOM
...
EOM
        ...
        it 'ceth download --purge of the entire data bag downloads everything' do
          ceth('download --purge /data_bags/x').should_succeed <<EOM
Created /data_bags/x/added.json
Updated /data_bags/x/modified.json
Deleted extra entry /data_bags/x/deleted.json (purge is on)
EOM
        ...
        context 'when cwd is the /data_bags directory' do
          cwd 'data_bags'
          it 'ceth download fails' do
            ceth('download').should_fail "FATAL: Must specify at least one argument.  If you want to download everything in this directory, type \"ceth download .\"\n", :stdout => /USAGE/
          end
          it 'ceth download --purge . downloads everything' do
            ceth('download --purge .').should_succeed <<EOM
...
EOM
    ...
    when_the_repository 'has a cookbook' do
      ...
      when_the_seth_server 'has a modified, added and deleted file for the cookbook' do
        ...
        it 'ceth download of a modified file succeeds' do
          ceth('download /cookbooks/x/metadata.rb').should_succeed "Updated /cookbooks/x/metadata.rb\n"
          ceth('diff --name-status /cookbooks').should_succeed <<EOM
D\t/cookbooks/x/y.rb
A\t/cookbooks/x/z.rb
EOM
        end
        it 'ceth download of a deleted file does nothing' do
          ceth('download /cookbooks/x/z.rb').should_succeed ''
          ceth('diff --name-status /cookbooks').should_succeed <<EOM
...
EOM
        ...
        it 'ceth download --purge of a deleted file succeeds' do
          ceth('download --purge /cookbooks/x/z.rb').should_succeed "Deleted extra entry /cookbooks/x/z.rb (purge is on)\n"
          ceth('diff --name-status /cookbooks').should_succeed <<EOM
M\t/cookbooks/x/metadata.rb
D\t/cookbooks/x/y.rb
EOM
        end
        it 'ceth download of an added file succeeds' do
          ceth('download /cookbooks/x/y.rb').should_succeed "Created /cookbooks/x/y.rb\n"
          ceth('diff --name-status /cookbooks').should_succeed <<EOM
M\t/cookbooks/x/metadata.rb
A\t/cookbooks/x/z.rb
EOM
        end
        it 'ceth download of the cookbook itself succeeds' do
          ceth('download /cookbooks/x').should_succeed <<EOM
...
EOM
        ...
        it 'ceth download --purge of the cookbook itself succeeds' do
          ceth('download --purge /cookbooks/x').should_succeed <<EOM
Updated /cookbooks/x/metadata.rb
Created /cookbooks/x/y.rb
...
EOM
    ...
    when_the_repository 'has a cookbook' do
      ...
      when_the_seth_server 'has a later version for the cookbook' do
        ...
        it 'ceth download /cookbooks/x downloads the latest version' do
          ceth('download --purge /cookbooks/x').should_succeed <<EOM
Updated /cookbooks/x/metadata.rb
...
EOM
      ...
      when_the_seth_server 'has an earlier version for the cookbook' do
        ...
        it 'ceth download /cookbooks/x downloads the updated file' do
          ceth('download --purge /cookbooks/x').should_succeed <<EOM
...
EOM
      ...
      when_the_seth_server 'has an earlier version for the cookbook, and no current version' do
        ...
        it 'ceth download /cookbooks/x downloads the old version' do
          ceth('download --purge /cookbooks/x').should_succeed <<EOM
Updated /cookbooks/x/metadata.rb
...
EOM
    ...
    when_the_repository 'has a cookbook' do
      ...
      when_the_seth_server 'has a modified, added and deleted file for the cookbook' do
        ...
        it 'ceth download of a modified file succeeds' do
          ceth('download /cookbooks/x-1.0.0/metadata.rb').should_succeed "Updated /cookbooks/x-1.0.0/metadata.rb\n"
          ceth('diff --name-status /cookbooks').should_succeed <<EOM
...
EOM
        ...
        it 'ceth download of a deleted file does nothing' do
          ceth('download /cookbooks/x-1.0.0/z.rb').should_succeed ''
          ceth('diff --name-status /cookbooks').should_succeed <<EOM
...
EOM
        ...
        it 'ceth download --purge of a deleted file succeeds' do
          ceth('download --purge /cookbooks/x-1.0.0/z.rb').should_succeed "Deleted extra entry /cookbooks/x-1.0.0/z.rb (purge is on)\n"
          ceth('diff --name-status /cookbooks').should_succeed <<EOM
M\t/cookbooks/x-1.0.0/metadata.rb
D\t/cookbooks/x-1.0.0/y.rb
EOM
        ...
        it 'ceth download of an added file succeeds' do
          ceth('download /cookbooks/x-1.0.0/y.rb').should_succeed "Created /cookbooks/x-1.0.0/y.rb\n"
          ceth('diff --name-status /cookbooks').should_succeed <<EOM
M\t/cookbooks/x-1.0.0/metadata.rb
A\t/cookbooks/x-1.0.0/z.rb
EOM
        ...
        it 'ceth download of the cookbook itself succeeds' do
          ceth('download /cookbooks/x-1.0.0').should_succeed <<EOM
...
EOM
        ...
        it 'ceth download --purge of the cookbook itself succeeds' do
          ceth('download --purge /cookbooks/x-1.0.0').should_succeed <<EOM
Updated /cookbooks/x-1.0.0/metadata.rb
Created /cookbooks/x-1.0.0/y.rb
...
EOM
    ...
    when_the_repository 'has a cookbook' do
      ...
      when_the_seth_server 'has a later version for the cookbook' do
        ...
        it 'ceth download /cookbooks/x downloads the latest version' do
          ceth('download --purge /cookbooks').should_succeed <<EOM
...
EOM
      ...
      when_the_seth_server 'has an earlier version for the cookbook' do
        cookbook 'x', '1.0.0', { 'metadata.rb' => 'version "1.0.0"', 'onlyin1.0.0.rb' => ''}
        cookbook 'x', '0.9.9', { 'metadata.rb' => 'version "0.9.9"', 'onlyin0.9.9.rb' => 'hi' }
        it 'ceth download /cookbooks downloads the updated file' do
          ceth('download --purge /cookbooks').should_succeed <<EOM
...
EOM
      ...
      when_the_seth_server 'has an earlier version for the cookbook, and no current version' do
        ...
        it 'ceth download --purge /cookbooks downloads the old version and deletes the new version' do
          ceth('download --purge /cookbooks').should_succeed <<EOM
Created /cookbooks/x-0.9.9
Created /cookbooks/x-0.9.9/metadata.rb
Created /cookbooks/x-0.9.9/onlyin0.9.9.rb
...
EOM
  ...
  when_the_seth_server 'has a cookbook' do
    ...
    when_the_repository 'is empty' do
      it 'ceth download /cookbooks/x signs all requests', :ruby_gte_19_only do
