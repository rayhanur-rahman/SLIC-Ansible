...
describe Seth::ceth::ConfigureClient do
  before do
    @ceth = Seth::ceth::ConfigureClient.new
  ...
  describe 'run' do
    it 'should print usage and exit when a directory is not provided' do
      ...
      @ceth.ui.should_receive(:fatal).with(/must provide the directory/)
      lambda {
        @ceth.run
